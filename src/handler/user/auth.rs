...
use macro_handler::{generate_handler, handler, perm, route, export};
...
pub mod handler {
    ...
    use crate::handler::{HttpError, HandlerError};
    use crate::handler::HandlerError::Conflict;
    use crate::utils::challenge::verify_captcha;

    #[perm]
    async fn verify_challenge(iden: &str, name: &str, email: &str, avatar: &str, password: &str, challenge_text: &str, verify: UserCreaterUserVerify) -> bool {
        let now = now_time!();
    ...
    #[export("message", "user")]
    async fn post_register(store: &mut impl ModelStore, iden: &str, name: &str, email: &str, avatar: &str, password: &str) -> ResultHandler<(String, UserNode)> {
        let user = UserRaw {
            iden: iden.to_string(),
            name: name.to_string(),
    ...
    async fn post_login(store: &mut impl ModelStore, user: &str, password: &str, long_token: Option<bool>) -> ResultHandler<(i64, UserNode, TokenNode)> {
    ...
    async fn get_before_register(dark_mode: bool, email: &str) -> ResultHandler<(String, String, i64)> {
        ...
        Ok((challenge_img, challenge_code, time.and_utc().timestamp_millis()))
    }
}
