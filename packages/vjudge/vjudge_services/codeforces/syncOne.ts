...
export const token = async (task: SyncTaskData): Promise<UniversalSubmission | null> => {
...
export const password = async (task: SyncTaskData): Promise<UniversalSubmission | null> => {
    const handle = task.vjudge_node.public.iden;
    const auth = task.vjudge_node.private.auth;
    const [id, contest_id] = task.range.split(":");
