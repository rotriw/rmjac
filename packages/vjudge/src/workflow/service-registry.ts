...
export class ServiceRegistry {
    ...
    async handleTask(request: TaskRequest): Promise<TaskResponse> {
        ...
        if (!service) {
            // 兼容旧协议的 platform/operation/method 组合
            const legacyKey = request.platform && request.operation && request.method
                ? `${request.platform}:${request.operation}:${request.method}`
                : null;
            if (legacyKey) {
                service = this.services.get(legacyKey);
            }
        }

        if (!service) {
            this.log("error", `[handleTask] Service not found: name="${serviceName}". Available: [${Array.from(this.services.keys()).join(", ")}]`);
            return {
                taskId,
                success: false,
