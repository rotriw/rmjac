...
use crate::graph::node::training::problem::{
    TrainingProblemNode, TrainingProblemNodePrivateRaw, TrainingProblemNodePublicRaw,
...
impl TrainingRepo {
        ...
        for edge in edges {
            ...
            match edge.problem_type {
                ...
                TrainingProblemType::Preset => {
                    let problem_iden =
                        if let Ok(iden_value) = get_node_id_iden(db, redis, next_node_id).await {
                            iden_value[0].clone()
                        } else {
                            "unknown".to_string()
                        };
                    result
                        .own_problem
                        .push(TrainingProblem::ExistTraining((next_node_id, problem_iden)));
                }
    ...
    pub async fn get(
        ...
        let edges =
            TrainingProblemEdgeQuery::get_order_asc_extend(training_node.node_id, db).await?;
        if edges.is_empty() {
            return Err(CoreError::NotFound(
                "No problem list found for this training".to_string(),
            ));
        }
    ...
    pub async fn set_order(
        ...
        for (id, order) in orders {
            let edge = TrainingProblemEdgeQuery::get_v_one_filter_extend(
                training_list_id,
                crate::db::entity::edge::training_problem::Column::VNodeId.eq(id),
                db,
            )
            .await;
            if let Ok(edge) = edge {
                edge.modify(
                    db,
                    crate::db::entity::edge::training_problem::Column::Order,
                    order,
                )
                .await?;
                continue;
    ...
    pub async fn purge(
        ...
        log::info!(
            "Starting to delete connections for training: {}#{}",
            user_iden,
            training_iden
        );
        ...
        if node_ids.is_empty() {
            return Err(CoreError::NotFound(
                "Cannot find training with this iden".to_string(),
            ));
        }
        ...
        log::info!(
            "Successfully deleted all connections for training: {}#{}",
            user_iden,
            training_iden
        );
        Ok(())
    ...
    pub async fn rm_all(
        ...
        log::info!(
            "Removing problem {} from training {}#{}",
            problem_iden,
            user_iden,
            training_iden
        );
        ...
        if problem_node_ids.is_empty() {
            return Err(CoreError::NotFound(
                "Cannot find problem with this iden".to_string(),
            ));
        }
        ...
        log::info!(
            "Successfully removed problem {} from training {}#{}",
            problem_iden,
            user_iden,
            training_iden
        );
    ...
    pub async fn set_desc(
        ...
        use crate::db::entity::node::training::Column::{DescriptionPrivate, DescriptionPublic};
        ...
        log::info!(
            "Successfully updated training {} descriptions",
            training_node_id
        );
        Ok(updated_training)
    }
    ...
    pub async fn status(
        ...
        let value = redis.exists(format!(
            "user_training_vis_{user_node_id}_{training_node_id}"
        ))?;
        if value {
        ...
        let _ = redis.set(
            format!("user_training_vis_{user_node_id}_{training_node_id}"),
            value,
        );
        let problem_list =
        ...
        for edge in problem_list {
            ...
            if node_type == "problem" || node_type == "problem_statement" {
                ...
                if let Ok(sub_status) = sub_status {
                    ...
                    let problem_status = if sub_status.completed_task == sub_status.total_task
                        && sub_status.total_task > 0
                    {
                        RecordStatus::Accepted
