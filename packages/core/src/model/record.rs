...
use sea_orm::{DatabaseConnection, ColumnTrait, EntityTrait, QueryFilter, Set, ActiveModelTrait, NotSet, QuerySelect};
...
impl RecordRepository {
    ...
    pub async fn by_node<T: IntoCondition>(
        ...
        match node_type.as_str() {
            ...
            "problem" | "problem_statement" => Self::by_problem(db, node_id, number_per_page, page, filter).await,
            _ => {
    ...
    pub async fn user_status(db: &DatabaseConnection, user_id: i64, problem_id: i64) -> Result<RecordStatus> {
...
impl Record {
    ...
    pub async fn set_status(
        ...
        log::trace!("Updating record {} status to {:?}", self.node_id, new_status);
        ...
        log::trace!("Successfully updated record {} status to {:?}", self.node_id, new_status);
        Ok(updated_record)
    }
    ...
    pub async fn set_score(
        ...
        log::trace!("Successfully updated record {} score to {}", self.node_id, score);
        Ok(updated_record)
    }
    ...
    pub async fn update_root_status(&self, store: &mut impl ModelStore, data: UpdateRecordRootStatusData) -> Result<()> {
    ...
    pub async fn status(
        ...
        log::trace!("Fetched {:?} socket edges for record_id {}", record_edges, self.node_id);
        ...
        let res = Self::get_record_status_with_now_id(
            redis,
            self.node_id,
            get_rt,
            get_rt,
        ...
        if force_refresh {
            self.update_root_status(store, UpdateRecordRootStatusData {
    ...
    pub async fn update_remote_status(
        ...
        log::debug!("Updating record {} status for statement {} with testcase root {}", self.node_id, statement_id, root_testcase_id);
        ...
        for testcase in testcase_list {
            ...
            use crate::db::entity::edge::judge::{Column, ActiveModel};
            log::debug!("Processing testcase {} for record {}", id, self.node_id);
            let edge_exist = JudgeEdgeQuery::get_v_one_filter_extend(testcase_node.node_id, Column::VNodeId.eq(self.node_id), &db).await;
        ...
        if !unused_data.is_empty() {
             store.get_redis().del(format!("graph_edge_testcase_{root_testcase_id}_v"))?;
        }
        ...
        for unused in unused_data {
            log::debug!("Unused testcase data for record/statement {}: {:?}, ", self.node_id, unused);
    ...
    fn get_record_status_with_now_id(
        ...
        for value in &now_value {
            ...
            let result_back = Self::get_record_status_with_now_id(redis, record_id, value, super_root_id);
