...
pub fn parse_handler_struct(item: &ItemStruct) -> Result<HandlerStruct, syn::Error> {
    ...
    if base_path.is_empty() {
        return Err(syn::Error::new_spanned(
            item,
            "handler struct must have #[handler(\"...\")] attribute",
        ));
    }
...
pub enum FuncType {
    ...
    PermFunction(PermFunction),
    HandlerFunction(HandlerFunction),
    OtherFunction,
}

/// 解析函数。
pub fn parse_func(func: &ItemFn) -> Result<FuncType, syn::Error> {
    let func_name = func.sig.ident.to_string();

    // 检查是否为handler函数
    if has_handler_attr(&func.attrs) {
        Ok(FuncType::HandlerFunction(parse_handler_function(func)?))
    }
    // 检查是否为before函数
    else if func_name.starts_with("before_") || has_from_path_attr(&func.attrs) {
        Ok(FuncType::BeforeFunction(parse_before_function(func)?))
    }
    // 检查是否为perm函数
    else if has_perm_attr(&func.attrs) {
        Ok(FuncType::PermFunction(parse_perm_function(func)?))
    } else {
        Ok(FuncType::OtherFunction)
    }
}
/// 解析before函数
fn parse_before_function(method: &ItemFn) -> Result<BeforeFunction, syn::Error> {
    ...
    for attr in &method.attrs {
        ...
        } else if attr.path().is_ident("require_login") {
            require_login = true;
        }
    }
...
/// 解析perm函数
fn parse_perm_function(method: &ItemFn) -> Result<PermFunction, syn::Error> {
    let name = method.sig.ident.clone();
    let params: Vec<Parameter> = method
        .sig
        .inputs
    ...
    let mut require_login = false;
    for attr in &method.attrs {
        if attr.path().is_ident("require_login") {
            require_login = true;
        }
    }
...
/// 解析handler函数
fn parse_handler_function(method: &ItemFn) -> Result<HandlerFunction, syn::Error> {
    let name = method.sig.ident.clone();
    ...
    for attr in &method.attrs {
        if attr.path().is_ident("route") {
            ...
            if let Some(first) = idents.into_iter().next() {
                ...
                perm_func = Some(Ident::from_string("perm")?);
            }
        } else if attr.path().is_ident("require_login") {
            require_login = true;
        }
...
pub fn parse_str_list_from_attr(attr: &Attribute) -> Result<Vec<syn::Lit>, syn::Error> {
    let mut lits = Vec::new();
    attr.parse_args_with(|input: syn::parse::ParseStream| {
        while !input.is_empty() {
            let ident: syn::Lit = input.parse()?;
            lits.push(ident);
