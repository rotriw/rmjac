...
pub fn build_dependency_graph(
    ...
    for before in before_funcs {
        ...
        // 获取from_path参数列表
        let from_path_params: HashSet<String> =
...
pub fn compute_execution_order(
    ...
    let graph = build_dependency_graph(before_funcs, handler, path_vars)?;
        ...
        for before_name in before_list {
            if let Some(before) = before_funcs.iter().find(|b| b.name == *before_name) {
                ...
                let from_path_vars: Vec<String> =
                    before.from_path.iter().map(|i| i.to_string()).collect();
                let all_path_vars_available = from_path_vars.iter().all(|v| path_vars.contains(v));
